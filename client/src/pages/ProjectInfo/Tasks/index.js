...
function Tasks({ project }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "createdAt",
      render: (text, record) => {
        const assignedDate = new Date(text);
        const day = assignedDate.getDate().toString().padStart(2, '0');
        const month = (assignedDate.getMonth() + 1).toString().padStart(2, '0');
        const year = assignedDate.getFullYear().toString().slice(-2);
